...
# Применение функции к каждому элементу в Series
y_new = y.apply(convert_values)



df = pd.concat([X, y_new], axis = 1)
...
model2 = EncoderClassifier(32000, EMBED_DIM, NUM_ENCODER_LAYERS, NUM_HEADS)
model2.load_state_dict(torch.load('Transformer_encoder_classifier.pth'))
...
lol.loc[:, 'predictions'] = np.round(all_outputs)
lol.to_csv('itog.csv')